...
class ConversationAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'conversation_type', 'get_participants', 'group',
        'last_message_at', 'is_active', 'created_at'
    ]
    list_filter = ['conversation_type', 'is_active', 'created_at']
    search_fields = [
        'name', 'group__name', 
        'user_a__username', 'user_b__username',
        'user_a__first_name', 'user_a__last_name',
        'user_b__first_name', 'user_b__last_name'
    ]
    readonly_fields = ['id', 'created_at', 'updated_at', 'avatar_preview', 'messages_count']
    
    fieldsets = (
        ('Thông tin cơ bản', {
            'fields': ('conversation_type', 'name', 'group')
        }),
        ('Người tham gia (Direct chat)', {
        ...
        ('Trạng thái', {
            'fields': ('is_active', 'last_message_at', 'messages_count')
        }),
        ('Thời gian', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
    ...
    def get_participants(self, obj):
        if obj.conversation_type == 'group':
            return f"Nhóm: {obj.group.name if obj.group else 'N/A'}"
        else:
            user_a_name = obj.user_a.get_full_name() or obj.user_a.username if obj.user_a else 'N/A'
            user_b_name = obj.user_b.get_full_name() or obj.user_b.username if obj.user_b else 'N/A'
            return f"{user_a_name} ↔ {user_b_name}"
    ...
    def messages_count(self, obj):
        ...
        url = reverse('admin:planpals_chatmessage_changelist') + f'?conversation__id__exact={obj.id}'
        return format_html('<a href="{}">{} tin nhắn</a>', url, count)
