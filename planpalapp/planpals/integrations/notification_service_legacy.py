...
class NotificationService(BaseService):
    ...
    def __init__(self):
        ...
        # Initialize firebase_admin if service account exists
        if FIREBASE_ADMIN_AVAILABLE and self.service_account_path:
            try:
                ...
                if sa_path.exists():
                    ...
                    if not firebase_admin._apps:
                        ...
                        logger.info('firebase_admin initialized with service account')
                else:
                    logger.warning('Firebase service account path not found: %s', self.service_account_path)
            except Exception as e:
                logger.error('Failed to initialize firebase_admin: %s', e)
    ...
    def send_push_notification_batch(self, fcm_tokens: List[str], title: str, 
        ...
        for i in range(0, len(fcm_tokens), self.batch_size):
            ...
            try:
                # If firebase_admin is available and initialized, prefer the Admin SDK (v1 API)
                if FIREBASE_ADMIN_AVAILABLE and firebase_admin._apps:
                    # Use send_multicast for batches
                    if len(batch_tokens) > 1:
                        ...
                        if response.failure_count:
                            # create pseudo-results for _handle_failed_tokens to reuse
                            results = []
                            for resp in response.responses:
                                if resp.success:
                                    results.append({})
                                else:
                                    # map to legacy style 'error' for compatibility with _handle_failed_token
                                    err = getattr(resp.exception, 'code', str(resp.exception))
                                    results.append({'error': err})
                            self._handle_failed_tokens(batch_tokens, results)
                    else:
                        # single token via Admin SDK
                        ...
                        try:
                            ...
                            # Admin SDK error - treat as failure
                            self._handle_failed_token(batch_tokens[0], {'error': str(e)})
                else:
                    # Legacy HTTP path
